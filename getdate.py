    ...
    if match:
        ...
        if unit.startswith("day"):
            ...
            while month < 1:
                ...
                year -= 1
            try:
                return datetime(year, month, now.day, now.hour, now.minute, now.second, tzinfo=LOCAL_TZ)
            except ValueError:
                day = min(now.day, 28)
                return datetime(year, month, day, now.hour, now.minute, now.second, tzinfo=LOCAL_TZ)
...
def _parse_ordinal_day(buf: str, now: datetime) -> Optional[datetime]:
    ...
    if match:
        ...
        ordinal = ORDINALS.get(ordinal_str, int(ordinal_match.group()) if ordinal_match else 1)
        target_day = DAYS[day_name]
        ...
        for d in range(1, 32):
            ...
            if dt.weekday() == target_day:
                count += 1
                if count == ordinal:
                    return datetime(year, month, d, now.hour, now.minute, now.second, tzinfo=LOCAL_TZ)
    ...
    if match:
        ...
        if last_occurrence:
            return datetime(year, month, last_occurrence, now.hour, now.minute, now.second, tzinfo=LOCAL_TZ)
...
def _parse_relative_unit(buf: str, now: datetime) -> Optional[datetime]:
    ...
    if match:
        ...
        if unit == "week":
            ...
            try:
                return datetime(year, now.month, now.day, now.hour, now.minute, now.second, tzinfo=LOCAL_TZ)
            except ValueError:
                day = min(now.day, 28)
                return datetime(year, now.month, day, now.hour, now.minute, now.second, tzinfo=LOCAL_TZ)
